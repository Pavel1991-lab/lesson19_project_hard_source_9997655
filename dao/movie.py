...
class MovieDAO:
    ...
    def get_all(self, filters):
        ...
        if status == "new" and page is not None:
            return self.session.query(Movie).order_by(Movie.year.desc()).paginate(page, per_page=12, error_out=False).items
        elif status == "new":
            return self.session.query(Movie).order_by(Movie.year.desc()).all()
        elif page is not None:
            return self.session.query(Movie).paginate(page, per_page=12, error_out=False).items
